...
public interface IntegrationRepository extends ReportPortalRepository<Integration, Long>, IntegrationRepositoryCustom {
	...
	@Query(value = "SELECT i.id, i.name, i.enabled, i.project_id, i.creator, i.creation_date, i.params, i.type FROM integration i WHERE i.params -> 'params' ->> 'identityProviderUrl' = :url", nativeQuery = true)
	Optional<Integration> findSamlByUrl(@Param("url") String url);
}
